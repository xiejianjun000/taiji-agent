...
class BaichuanAdapter(LLMAdapter):
    ...
    Usage:
        config = ModelConfig(provider=LLMProvider.BAICHUAN, model_id="baichuan4", ...)
        adapter = BaichuanAdapter(config)
    ...
    def chat(self, messages: list[dict[str, str]], **kwargs: Any) -> str:
        ...
        try:
            ...
            if "error" in result:
                raise RuntimeError(f"百川API错误: {result['error']}")

            return result.get("choices", [{}])[0].get("message", {}).get("content", "")

        except requests.RequestException as e:
            raise RuntimeError(f"百川请求失败: {e}")
    ...
    def stream_chat(self, messages: list[dict[str, str]], **kwargs: Any) -> Iterator[str]:
        ...
        try:
            ...
            raise RuntimeError(f"百川流式请求失败: {e}")
    ...
    def get_embedding(self, text: str, **kwargs: Any) -> list[float]:
        ...
        embed_url = "https://api.baichuan-ai.com/v1/embeddings"
        ...
        try:
            ...
            raise RuntimeError(f"百川嵌入请求失败: {e}")
