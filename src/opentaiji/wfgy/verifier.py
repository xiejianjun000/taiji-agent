...
class WFGYVerifier:
    ...
    def __init__(
        ...
        self.rules = rules or []
        self.knowledge_base: dict[str, WFGYKnowledgeEntry] = {
            entry.symbol: entry for entry in (knowledge_base or [])
        }
    ...
    def _verify(self, content: str) -> WFGYVerificationResult:
        ...
        for compiled, rule in self._compiled_rules:
            ...
            if matches and not rule.expected:
                violations.append(
                    rule.violation_message or f"Rule '{rule.name}' violated"
                )
            ...
            if symbol in content:
                ...
                for forbidden in entry.forbidden_contexts:
                    if forbidden in content:
                        violations.append(
                            f"Symbol '{symbol}' used in forbidden context: {forbidden}"
                        )
                        total_weight += 0.5
...
class HallucinationDetector:
    ...
    def detect(self, content: str) -> float:
        ...
        uncertain_count = sum(
            len(re.findall(p, content)) for p in uncertain_patterns
        )
        ...
        absolute_count = sum(
            len(re.findall(p, content)) for p in absolute_patterns
        )
