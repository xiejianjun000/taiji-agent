...
class TaijiAgent(BaseAgent):
    ...
    def __init__(
        ...
        from opentaiji import TaijiAgent as CoreAgent, AgentConfig
...
class MultiAgentCoordinator:
    ...
    async def execute_parallel(
        ...
        return [r if isinstance(r, AgentTask) else None for r in results]
    ...
    async def execute_hierarchical(
        ...
        try:
            ...
            if len(subtasks) == 1:
                result = await self._execute_single_task(subtasks[0], agent_factory)
    ...
    async def execute_debate(
        ...
        return {
            ...
            "consensus": votes[winner_id] > len(agents) / 2,
        }
    ...
    async def execute_consensus(
        ...
        proposals = await asyncio.gather(
            *[agent.process(
                AgentTask(description=f"提出对 '{topic}' 的解决方案")
              ) for agent in agents],
            return_exceptions=True
        )
        
        votes = {i: 0 for i in range(len(agents))}
        ...
        return {
            ...
            "consensus_proposal": str(proposals[consensus_idx]) if not isinstance(proposals[consensus_idx], Exception) else None,
            "votes": votes,
        }
    ...
    async def broadcast(
        ...
        await asyncio.gather(
            *[self.route_message(message) for _ in target_agents],
            return_exceptions=True
        )
    ...
    async def _default_decompose(self, task: AgentTask) -> List[AgentTask]:
        ...
        for key, subs in keywords.items():
            if key in task.description:
                return [
                    AgentTask(description=sub, dependencies=[task.id])
                    for sub in subs
                ]
